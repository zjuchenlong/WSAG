...
class YouwikiHow_MILNCE(data.Dataset):
    ...
    def __init__(self, split, config):
        super(YouwikiHow_MILNCE, self).__init__()
        ...
        MILNCE_file = open(os.path.join('./baselines/MILNCE-WSTAN', f'{self.split}.pkl'), 'rb')
        self.MILNCE_annos = pkl.load(MILNCE_file)
        ...
        for task_id, anno in tqdm(self.raw_annos.items()):
            ...
            for vid in task_videos:
                if vid in ['bHHB3u9pZj4', 'ZJNm0DaKLYs']: # no s3d features from ht100m
                    ...
                    ###### use the pre-obtained MIL-NCE results
                    video_task_key = str(vid) + '_' + str(task_id)
                    assert video_task_key in self.MILNCE_annos.keys()
                    MILNCE_score = self.MILNCE_annos[video_task_key]

                    # select sentences based on threshold
                    MILNCE_mean_score = MILNCE_score.mean(-1).mean(-1)
                    MILNCE_mask = MILNCE_mean_score > config.DATASET.MILNCE_THRE

                    # assert len(sentences) == len(MILNCE_mask) # only keep top20
    ...
    def __getitem__(self, index):
        ...
        MILNCE_mask = self.annotations[index]['MILNCE_mask']

        if self.split == 'train':
            MILNCE_idx = MILNCE_mask.nonzero()
            if len(MILNCE_idx) > 0:
                sentences = [sentences[i] for i in MILNCE_idx.flatten()]
            else:
                #### random sample a sentence ###
                rand_idx = random.choice(range(0, len(sentences)))
                sentences = sentences[rand_idx:rand_idx+1]
