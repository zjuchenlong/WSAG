...
def didemo_iou(pred, gt):
    intersection = max(0, min(pred[1], gt[1]) + 1 - max(pred[0], gt[0]))
    union = max(pred[1], gt[1]) + 1 - min(pred[0], gt[0])
    return float(intersection)/union


def eval_didemo(segments, data, verbose=True):
    # print('didemo')
    average_ranks = []
    average_iou = []
    for s, d in zip(segments, data):
      pred = s[0]
      ious = [didemo_iou(pred, [t[0], t[1] + 1]) for t in d['times']]
      average_iou.append(np.mean(np.sort(ious)[-3:]))
      ranks = [rank(s, [t[0], t[1] + 1]) for t in d['times']]
    ...
    miou = np.mean(average_iou)
    eval_result = np.array([[rank1, rank5]])
